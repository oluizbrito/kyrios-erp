...
uses
  ...
  frxDBSet, frxExportPDF, Vcl.Tabs, JvComponentBase, JvEnterTab, dxSkinsCore,
...
type
  TfrmCadOS = class(TForm)
    ...
    qrySomaPecasSUBTOTAL: TBCDField;
    Label58: TLabel;
    qrySomaServivosSUBTOTAL: TBCDField;
...
procedure TfrmCadOS.cxAbrirClick(Sender: TObject);
begin
  if not cxAbrir.Enabled then
    exit;
...
procedure TfrmCadOS.cxFecharClick(Sender: TObject);
begin
  if not cxFechar.Enabled then
    exit;
...
procedure TfrmCadOS.cxFinalizarClick(Sender: TObject);
begin
...
procedure TfrmCadOS.cxGravarClick(Sender: TObject);
begin
...
procedure TfrmCadOS.cxPessoasClick(Sender: TObject);
begin
   try
...
procedure TfrmCadOS.cxProdutosClick(Sender: TObject);
var
  idProduto: Integer;
begin
...
procedure TfrmCadOS.cxSairClick(Sender: TObject);
begin
  Close;
end;
...
begin
  ...
  then
  begin
    cxAbrir.Enabled := true;
    cxFechar.Enabled := false;
  end;

  if qryOS_DetalheSITUACAO.AsString = 'A' then
  begin
    cxAbrir.Enabled := false;
    cxFechar.Enabled := true;
  end;
