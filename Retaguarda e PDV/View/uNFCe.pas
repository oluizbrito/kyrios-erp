...
uses
  ...
  System.Classes, Vcl.Graphics, System.Math, dateutils, ACBrDFeUtil, StrUtils,
  ...
  ACBrDANFCeFortesFrA4, frxExportXLS, System.Actions, Vcl.ActnList, cxGraphics,
...
type
  TfrmNFCe = class(TForm)
    ...
    qryItemCEST: TStringField;
    qryItemBLOQUEADO: TStringField;
    qryItemVICMSDESON: TBCDField;
    qryItemALIQICMSDESON: TBCDField;
    qryItemFK_GRADE: TIntegerField;
    qryDocumentoTOTAL: TBCDField;
    qryDocumentoSUBTOTAL: TBCDField;
    qryDocumentoDESCONTO: TBCDField;
    qryDocumentoACRESCIMO: TBCDField;
    Panel13: TPanel;
    ...
    cxInutilizar: TcxButton;
    cxPDF: TcxButton;
    Panel12: TPanel;
    cxSair: TcxButton;
    PopupMenu_NFCe: TPopupMenu;
    ...
    QRYNFCE_DVALOR_ISS.Value := 0;
      ...
      begin
            raise Exception.CreateFmt
              ('NCM do produto "%s" - "%s" Não foi encontrado!',
              [QRYNFCE_DID_PRODUTO.AsString,
              QRYNFCE_DDESCRICAO.AsString]);
      end else
      begin
        QRYNFCE_DTRIB_MUN.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTMUNICIPAL.Value, '.',  ',', [])) * qryItemTOTAL.AsFloat) / 100, -2);
        QRYNFCE_DTRIB_EST.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTESTADUAL.Value, '.', ',',  [])) * qryItemTOTAL.AsFloat) / 100, -2);

        QRYNFCE_DTRIB_FED.AsFloat := 0;
        QRYNFCE_DTRIB_IMP.AsFloat := 0;

        if AnsiMatchStr(qryProdutoCSTICMS.AsString[1], ['0', '3', '4', '5']) then
          QRYNFCE_DTRIB_FED.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTNACIONALFEDERAL.Value,  '.', ',', [])) * qryItemTOTAL.AsFloat) / 100, -2)
        else
          QRYNFCE_DTRIB_IMP.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTIMPORTADOSFEDERAL.Value,  '.', ',', [])) * qryItemTOTAL.AsFloat) / 100, -2);
      end;
      if (QRYNFCE_D.State in dsEditModes) then
      QRYNFCE_D.Post;
...
procedure TfrmNFCe.Imprimir1Click(Sender: TObject);
begin
...
procedure TfrmNFCe.Visualizar1Click(Sender: TObject);
var
 XMLContent,CaminhoXML: string;
begin
 try
// Verifica se há registros na tabela
    if not qryVenda.IsEmpty then
    begin
      // Obtém o XML do campo da tabela
      XMLContent := qryVenda.FieldByName('XML').AsString;

      if Trim(XMLContent) = '' then
      begin
        ShowMessage('O XML da NFC-e não está disponível.');
        Exit;
      end;


    ACBrNFe1.NotasFiscais.Clear;
    ACBrNFe1.NotasFiscais.LoadFromString(XMLContent);
    ...
    on E: Exception do
      ShowMessage('Erro ao visualizar a NFC-e: ' + E.Message);
  end;
...
var
  ...
  Wdiferencavalor, WtotalFpg, WtotalNfce: Double; // Troco UP06
begin
  try
    ...
    WtotalNfce := qryNfce_Mtotal.asFloat; //
    ...
    begin
      ...
      begin
        ...
        { TODO : Validação do troco.}
        Wtotalfpg := qrytotalfpgvalor.asfloat; //
       if (qryTotalfpgValor.asFloat <> dados.qryNfce_Mtotal.asFloat) then //
        begin
          if WtotalFPG > WtotalNFCE then
          WdiferencaValor := (WtotalFPG - WtotalNFCE)
          else
          WdiferencaValor := (WtotalNFCE - WtotalFPG);
        end; //
        ...
        begin
          ...
          begin
            if (UpperCase(qryTotalFPGTIPO.Value) = 'D') then   // DINHEIRO E DEPOSITO
            begin
              with pag.Add do
              begin
                tPag := fpDinheiro;
          ...
          if UpperCase(qryTotalFPGTIPO.Value) = 'I' then // Pix
          begin
            if qryTotalFPGVALOR.Value > 0 then
            begin
              with pag.add do
      ...
      if (WtotalFPG <> WtotalNfce) then // troco
      pag.vTroco := WdiferencaValor
      else
...
procedure TfrmNFCe.cxImprimirClick(Sender: TObject);
begin
PopupMenu_NFCe.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TfrmNFCe.cxSairClick(Sender: TObject);
begin
    close;
end;
