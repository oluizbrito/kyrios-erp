    ...
    begin
      try
        ...
        if PageControl2.ActivePage = TabPDV then
          ...
          EdtProdutoP.Text := (copy(Descricao, 1, Pos('*', Descricao) )) +  qryPesqProdDESCRICAO.Value;
          FrmPDV.EdtProdutoP.OnChange := FrmPDV.EdtProdutoPChange;

        if PageControl2.ActivePage = tabRestaurante then
          FrmPDV.EdtProdutoR.OnChange := nil;
          edtProdutoR.Text := (copy(Descricao, 1, Pos('*', Descricao) )) +  qryPesqProdDESCRICAO.Value;
          FrmPDV.EdtProdutoR.OnChange := FrmPDV.EdtProdutoRChange;

        if PageControl2.ActivePage = tabDelivery then
          FrmPDV.EdtProdutoD.OnChange := nil;
          edtProdutoD.Text := (copy(Descricao, 1, Pos('*', Descricao) )) +  qryPesqProdDESCRICAO.Value;
...
begin
  ...
  qryPesqProd.ResourceOptions.DirectExecute := True; // Evita cache e otimiza execução
...
begin
  try
    ...
    ACBrBAL1.Device.HandShake := TACBrHandShake(
      Dados.qryTerminalBALANCA_HANDSHAKING.AsInteger
    );
    ...
    ACBrBAL1.Device.Data := StrToIntDef(Dados.qryTerminalBALANCA_DATABITS.Value, 5);
    ACBrBAL1.Device.Baud := StrToIntDef(Dados.qryTerminalBALANCA_VELOCIDADE.Value, 9600);
    ...
    begin
      Application.MessageBox(
        PChar('Erro ao configurar a balança:' + sLineBreak + E.Message),
  ...
  begin
    try
      ...
      begin
        ...
        raise Exception.CreateFmt('Erro ao inserir item no PDV: %s',
          [e.Message]);
      end;
    end;
    ...
    try
      ...
      Dados.qryExecute.ParamByName('E_MEDIO').Value :=
        qryPesqProdE_MEDIO.AsFloat;
      Dados.qryExecute.ParamByName('FK_MESA').Clear;
      if PageControl2.ActivePage = tabRestaurante then
...
begin
  ...
  qryPesqProd.SQL.Text := StringReplace(vSql, '/*where*/',
   ' AND (PRO.DESCRICAO LIKE :DESCRICAO) ORDER BY DESCRICAO',
    [rfReplaceAll]);
