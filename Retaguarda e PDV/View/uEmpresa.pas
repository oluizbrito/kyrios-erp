...
procedure TfrmEmpresa.DBRadioGroup2Change(Sender: TObject);
begin
    case DBRadioGroup2.Value.ToInteger of
    0:
      PageControl4.ActivePage :=  tabBancoBrasil;
    1:
      PageControl4.ActivePage :=  tabGerenciaNet;
    2:
      PageControl4.ActivePage :=  tabSantander;
    3:
      PageControl4.ActivePage :=  tabSicoob;
    4:
  ...
  begin
    try
      ...
      qryEmpresaID_CIDADE.Value := dados.BuscaCodigoIbge(qryEmpresaCIDADE.Value,
        qryEmpresaUF.Value);
      qryEmpresaCNAE.value := dmrotinas.Pessoa.cnae;
      qryEmpresaCOMPLEMENTO.Value := UpperCase(dmrotinas.Pessoa.complemento);
      qryEmpresaFONE.value := dmrotinas.Pessoa.telefone;
      qryEmpresaRESPONSAVEL_EMPRESA.value := UpperCase(dmrotinas.Pessoa.socioAdministrador);
