...
uses
  ...
  ACBrDFe, ACBrMail, ACBrCTeDACTEClass, ACBrCTeDACTeRLClass, ACBrCTe,
  Vcl.Grids, Vcl.DBGrids, ACBrDFeReport, JvExDBGrids, JvDBGrid, cxGraphics,
...
type
  TfrmCadCTe = class(TForm)
    ...
    qryCte_SeguradoraVALOR_SEGURADO: TBCDField;
    Panel5: TPanel;
...
begin
  ...
  begin
    ...
    //emit.CRT := dados.qryEmpresaCRT.Value;
    // Dados do Remetente
...
//    vPrest.vRec := qryCTE_MVALOR_SERVICO.AsFloat;
    ...
    begin
      xNome := 'VALOR LIQUIDO PRESTA��O DE SERVI�O';
    ...
    Imp.infTribFed.vPIS := (qryCTE_MVALOR_SERVICO.AsFloat * dados.qryConfigCTE_PIS.AsFloat) / 100;
    Imp.infTribFed.vCOFINS := (qryCTE_MVALOR_SERVICO.AsFloat * dados.qryConfigCTE_COFINS.AsFloat) / 100;
    ...
    begin
      // Inicializa a variável FResponsavel
      FResponsavel := 0;
      ...
      begin
        // Se o valor não for válido, usa Emitente como padrão
        FResponsavel := 4;
        ShowMessage('Valor inválido para Responsável do Seguro. Usando "Emitente do CT-e" como padrão.');
      end;
...
if (trim(DBLookupComboboxEh4.Text) = '') then
  begin
    showMessage('Informe a Natureza de opera��o, antes de importar');
...
begin
  ...
  begin // verifica se j� foi transmitido
    if not(qryCTE_MCHAVE.IsNull) then
    begin
      showMessage('Conhecimento de frete j� transmitido!');
      qryCTE_M.Edit;
  ...
  qryCTE_M.Edit;
  qryCTE_MCHAVE.Value := copy(dmCTe.ACBrCTe.Conhecimentos.Items[0].CTe.infCTe.ID, 4, 100);
  ...
  begin
    qryCTE_M.Edit;
    qryCTE_MPROTOCOLO.Value := dmCTe.ACBrCTe.Conhecimentos.Items[0].CTe.procCTe.nProt;
    qryCTE_MXML.Value := dmCTe.ACBrCTe.Conhecimentos.Items[0].XML;
...
procedure TfrmCadCTe.DBEdit16Exit(Sender: TObject);
begin
  CalcularICMS();
end;

procedure TfrmCadCTe.DBEdit31Exit(Sender: TObject);
begin
  CalcularICMS();
end;

procedure TfrmCadCTe.DBEdit32Exit(Sender: TObject);
begin
  CalcularICMS();
end;

procedure TfrmCadCTe.CalcularICMS();
begin
  if (qryCTE_MCST_ICMS.AsString = '00') or (qryCTE_MCST_ICMS.AsString = '99') then
  begin
    if (qryCTE_MALIQUOTA_ICMS.AsFloat <> 0) and (qryCTE_MBASE_ICMS.AsFloat<> 0)  then
      qryCTE_MVALOR_ICMS.AsFloat := RoundABNT((qryCTE_MALIQUOTA_ICMS.AsFloat *
                                                qryCTE_MBASE_ICMS.AsFloat) / 100, -2)
    else
      qryCTE_MVALOR_ICMS.AsFloat := 0;
  end
  else
     qryCTE_MVALOR_ICMS.AsFloat := 0;
end;
...
begin
  ...
  begin
    If Application.messageBox('Tem certeza que Excluir Item?', 'Confirma��o',
...
begin
  qryCTE_DTOTAL.Value := SimpleRoundTo(qryCTE_DPRECO.AsFloat * qryCTE_DQTD.AsFloat, -2);
end;
...
begin
  ...
  begin
    ...
    begin
      showMessage('J� existe CTe com esta numera��o!');
      Abort;
    end;
