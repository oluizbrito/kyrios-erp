...
uses
  ...
  cxPC, Vcl.Grids, Vcl.DBGrids, Vcl.Menus, cxButtons, udadosSped, System.Skia,
  Vcl.Skia;
type
  ...
  begin
    ...
    begin
      ...
      if (dtIni.Date <= strtodate('31/12/2022')) then
        COD_VER := vlVersao115;
      if (dtIni.Date <= strtodate('31/12/2023')) then
        COD_VER := vlVersao116;
      if (dtIni.Date <= strtodate('31/12/2024')) then
        COD_VER := vlVersao117;
      if (dtIni.Date <= strtodate('31/12/2025')) then
        COD_VER := vlVersao118;
...
begin
 try
    ShowMessage
      ('Atenção: Ao gerar o SPED Fiscal, o processo pode levar alguns minutos.'
      + #13 + 'O sistema não está travado, ele está apenas buscando e processando dados importantes no banco.'
      + #13 + 'Por favor, aguarde até que a geração seja concluída.');
