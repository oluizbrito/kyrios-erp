...
type
  ...
  TfrmFechaVenda = class(TForm)
    ...
    qryVendasFPGVIRTUAL_ATALHO: TStringField;
    qryClienteEMAIL1: TStringField;
    qryVendasFPGINDICE: TStringField;
    qryVendasFPGFEZ_TEF: TStringField;
    qryVendasFPGNSU: TStringField;
    qryVendasFPGREDE: TStringField;
    qryVendasFPGREDECNPJ: TStringField;
    qryVendasFPGCODIGOAUTORIZACAO: TStringField;
    qryVendasFPGBIN: TStringField;
    qryVendasFPGDONOCARTAO: TStringField;
    qryVendasFPGDATAEXPIRACAO: TStringField;
    qryVendasFPGPARCELAS: TIntegerField;
    qryVendasFPGCODIGOPAGAMENTO: TIntegerField;
    qryVendasFPGVALORPAGAMENTO: TBCDField;
    qryVendasFPGID_FILA: TIntegerField;
    qryVendasFPGTIPO_TEF: TStringField;
...
begin
  try
    ...
    SL.Add('CNPJ: ' + Dados.FormataCPFCNPJ(dados.qryEmpresaCnpj.AsString));
  ...
  try
    ...
    begin
      ...
      begin
            ...
            PageControl1.ActivePage:= TabPrecoNormal;
      ...
      begin
        dados.QRYNFCE_DTRIB_MUN.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTMUNICIPAL.Value, '.',  ',', [])) * qryItemTOTAL.AsFloat) / 100, -2);
        dados.QRYNFCE_DTRIB_EST.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTESTADUAL.Value, '.', ',',  [])) * qryItemTOTAL.AsFloat) / 100, -2);

        dados.QRYNFCE_DTRIB_FED.AsFloat := 0;
        dados.QRYNFCE_DTRIB_IMP.AsFloat := 0;

        if AnsiMatchStr(qryProdutoCSTICMS.AsString[1], ['0', '3', '4', '5']) then
          dados.QRYNFCE_DTRIB_FED.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTNACIONALFEDERAL.Value,  '.', ',', [])) * qryItemTOTAL.AsFloat) / 100, -2)
        else
          dados.QRYNFCE_DTRIB_IMP.AsFloat := SimpleRoundTo((strtofloat(stringreplace(qryIBPTIMPORTADOSFEDERAL.Value,  '.', ',', [])) * qryItemTOTAL.AsFloat) / 100, -2);
      end;
