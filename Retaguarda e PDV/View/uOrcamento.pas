...
uses
  ...
  frxExportXLS, Vcl.Imaging.pngimage, frxExportImage, dxSkinsCore,
...
type
  TfrmOrcamento = class(TForm)
    ...
    procedure cxViewClick(Sender: TObject);
...
procedure TfrmOrcamento.cxAlterarClick(Sender: TObject);
begin
...
procedure TfrmOrcamento.cxAtualizarClick(Sender: TObject);
var
  codigo: Integer;
begin
  try
...
procedure TfrmOrcamento.cxButton1Click(Sender: TObject);
begin
   close;
end;

procedure TfrmOrcamento.cxCancelarClick(Sender: TObject);
begin
...
procedure TfrmOrcamento.cxEmailClick(Sender: TObject);
begin
  if Dados.qryOrcamento.IsEmpty then
    exit;
  try
    ...
    frmEmail.Release;
    cxEmail.Enabled := true;
    Dados.qryPV.Filtered := false;
  end;
end;
procedure TfrmOrcamento.cxImpClick(Sender: TObject);
begin
  pnImpressao.Visible := true;
end;
procedure TfrmOrcamento.cxNovoClick(Sender: TObject);
begin
...
procedure TfrmOrcamento.cxViewClick(Sender: TObject);
begin
  if Dados.qryOrcamento.IsEmpty then
    exit;
  try
    cxView.Enabled := false;
    FrmCadOrcamento := TFrmCadOrcamento.Create(Application);
    with FrmCadOrcamento do
    begin
...
procedure TfrmOrcamento.cxWhatsClick(Sender: TObject);
var
