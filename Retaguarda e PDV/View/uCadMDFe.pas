...
uses
  ...
  JvExDBGrids, JvDBGrid, Vcl.Menus, dxSkinsCore, dxSkinDevExpressDarkStyle,
...
type
  TfrmCadMDFe = class(TForm)
    ...
    cxVeiculo: TcxButton;
    cxTransp: TcxButton;
    qryMDFe_SeguradoraVALOR_SEGURADO: TBCDField;
...
procedure TfrmCadMDFe.cxImportarClick(Sender: TObject);
begin
  try
...
procedure TfrmCadMDFe.cxImprimirClick(Sender: TObject);
begin
...
procedure TfrmCadMDFe.cxPessoasClick(Sender: TObject);
begin
  try
...
procedure TfrmCadMDFe.cxTransmitirClick(Sender: TObject);
var
...
begin
  try
    ...
    begin
      ...
      begin
        showMessage('MDFe j� transmitido!');
        qryMDFE_M.Edit;
...
procedure TfrmCadMDFe.cxTranspClick(Sender: TObject);
begin
  try
...
procedure TfrmCadMDFe.cxVeiculoClick(Sender: TObject);
begin
  try
...
begin
  ...
  begin
    ...
    begin
      raise Exception.Create('Ve�culo n�o foi encontrado!');
      exit;
    end;
    ...
    begin
      raise Exception.Create('Motorista n�o foi encontrado!');
      exit;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FResponsavel := 4;
          ShowMessage('Valor inválido para Responsável do Seguro. Usando "Emitente" como padrão.');
        end;
...
begin
  ...
  cxGravar.Caption := 'F2' + sLineBreak + 'Gravar';
  ...
  cxPessoas.Caption := 'F6' + sLineBreak + 'Destinat�rio';
  cxTransp.Caption := 'F7' + sLineBreak + 'Transportador';
  cxVeiculo.Caption := 'F8' + sLineBreak + 'Ve�culos';
...
begin
  ...
  begin
    ...
    begin
      showMessage('J� existe MDFe com esta numera��o!');
      Abort;
    end;
...
begin
  if (qryMDFE_MSITUACAO.Value = 'T') then
    cxGravar.Enabled := false;
  cxImprimir.Enabled := (qryMDFE_MSITUACAO.Value = 'T');
  cxImportar.Enabled := qryMDFE_MSITUACAO.Value = 'A';
