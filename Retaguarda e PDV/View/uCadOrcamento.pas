...
type
  TfrmCadOrcamento = class(TForm)
    ...
    procedure qryItensOPRECOValidate(Sender: TField);
...
procedure TfrmCadOrcamento.cxFinalizarClick(Sender: TObject);
begin
...
procedure TfrmCadOrcamento.cxGravarClick(Sender: TObject);
begin
  if (qryOrcamentoFKVENDEDOR.IsNull) then
  begin
    ShowMessage('Informe o Vendedor!');
...
procedure TfrmCadOrcamento.cxPessoasClick(Sender: TObject);
begin
  try
...
procedure TfrmCadOrcamento.cxProdutosClick(Sender: TObject);
begin
   try
...
procedure TfrmCadOrcamento.cxSairClick(Sender: TObject);
begin
   Close;
end;
...
procedure TfrmCadOrcamento.qryItensOPRECOValidate(Sender: TField);
begin

  if qryItensOPRECO.Value < 0 then
...
procedure TfrmCadOrcamento.DBEdit2Change(Sender: TObject);
begin
   ...
   if qryItensO.State in dsEditModes then
    begin
      ...
      if qryItensOQTD.Value > 99999 then
