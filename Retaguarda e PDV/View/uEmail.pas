...
begin
  ...
  case aStatus of
    ...
    pmsConfigHeaders:
      caption := 'Enviar Email: Configurando o cabeçalho do e-mail.';
    pmsLoginSMTP:
      caption := 'Enviar Email: Logando no servidor de e-mail.';
    pmsStartSends:
      caption := 'Enviar Email: Iniciando os envios.';
    pmsSendTo:
      caption := 'Enviar Email: Processando lista de destinatários.';
    pmsSendCC:
...
begin
  try
    ...
    ShowMessage('Não foi possível selecionar arquivo!');
  end;
end;

procedure TFrmEmail.BtnExcluirClick(Sender: TObject);
begin
  if Application.messageBox('Deseja Excluir Anexo?', 'Confirmação', mb_YesNo) = mrYes
  then
