...
begin
  ...
  if trim(edtReferencia.Text) <> '' then
    Filtro := Filtro + ' AND pro.referencia LIKE ' +  // Corrigido para LIKE
      QuotedStr(parte + edtReferencia.Text + '%');  // Adicionado % para busca parcial

  if trim(edtAplicacao.Text) <> '' then
    Filtro := Filtro + ' AND pro.localizacao LIKE ' +
      QuotedStr(parte + edtAplicacao.Text + '%');  // Usando LIKE para consistência
  ...
  // Garantindo que há um espaço antes da cláusula ORDER BY
  if dados.qryEmpresaCHECA_ESTOQUE_FISCAL.Value = 'N' then
    qryPesquisa.SQL.Text :=
      'SELECT pro.codigo, pro.codbarra, pro.descricao, pro.referencia, pro.pr_venda, pro.qtd_atual, ' +
      ' ma.descricao AS marca ' +
      ' FROM produto pro LEFT JOIN marca ma ON ma.codigo = pro.fk_marca ' +
      ' WHERE pro.QTD_ATUAL > 0 AND pro.ativo = ''S'' ' + Filtro +
      ' ORDER BY pro.descricao'
  else
    qryPesquisa.SQL.Text :=
      'SELECT pro.codigo, pro.codbarra, pro.descricao, pro.referencia, pro.pr_venda, pro.qtd_atual, ' +
