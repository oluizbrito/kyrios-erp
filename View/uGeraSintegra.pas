...
function GerarReg54(cpfcnpj, modelo, serie, numero, cfop, cst, csosn,
  numeroitem, codigo: string; quantidade, valor, ValorDescontoDespesa,
  basedecalculo, basest, valoripi, aliquota: real): string;
...
begin
  ...
  if length(csosn) = 3 then
    wregistro := wregistro + PadRight(csosn, 3)
  else
...
begin
  ...
  wregistro := wregistro + '99' +
    TBStrZero(IntToStr(qtd50 + qtd51 + qtd53 + qtd54 + (qtd61 + qtd61r) + qtd74
    + qtd75 + wtotal88 + WTotal90), 8);
