...
begin
  ...
  begin
    ...
    begin
      if dadosweb.cdsPessoas.Locate('CNPJ',
        tirapontos(qryPessoas.FieldByName('CNPJ').Value), []) then
      ...
      if qryPessoas.FieldByName('CELULAR1').IsNull then
        dadosweb.cdsPessoas.FieldByName('CELULAR1').Value := '0000000000'
      else
        begin
      ...
      dadosweb.cdsPessoas.FieldByName('CODIGOLOCAL').Value :=
        qryPessoas.FieldByName('CODIGO').Value;
...
begin
  ...
  begin
    ...
    begin
      if dadosweb.cdsProdutos.Locate('CODIGO', qryProdutos.FieldByName('CODIGO')
        .Value, []) then
